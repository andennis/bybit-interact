...
def test_get_last_price(bybit: Bybit):
    ...
    with patch.object(
        ...
        mock_get_tickers.assert_called_once_with(category=Market.LINEAR.value, symbol=TradePair.TON_USDT.value)
...
@pytest.mark.parametrize("last_price, trigger_price, trigger_direction, order_price", [
    (5.5, 5.4, TriggerDirection.FALLS, 5.4),
    (5.5, 5.45, TriggerDirection.FALLS, 5.4),
    (5.5, 5.6, TriggerDirection.RAISES, 5.55),
])
def test_buy_limit_conditional(bybit: Bybit, last_price: float, trigger_price: float, trigger_direction: TriggerDirection, order_price: float):
    data = {"retCode": 0, "retMsg": "OK"}
    qyt = 2.55
    tp_sl = TPSLProperties(take_profit=order_price + 0.5, stop_loss=order_price - 0.1)
    with (patch.object(bybit.trade.derivatives, "get_last_price", return_value=last_price),
          patch.object(bybit.trade.derivatives.session, "place_order", return_value=data) as mock_place_order):
...
def test_sell_limit_conditional(bybit: Bybit, last_price: float, trigger_price: float, trigger_direction: TriggerDirection, order_price: float):
    data = {"retCode": 0, "retMsg": "OK"}
    qyt = 2.55
    tp_sl = TPSLProperties(take_profit=order_price - 0.5, stop_loss=order_price + 0.1)
    with (patch.object(bybit.trade.derivatives, "get_last_price", return_value=last_price),
